...
const Navbar = () => {
  ...
  return (
    <>
    <nav className='conatiner mx-auto flex items-center justify-between py-4 px-6'>
        {/* Left-logo*/}
        <div>
        ...
        <div className='flex items-center space-x-4'>
          ...
          className='block bg-black px-2 text-sm text-white'>Admin</Link>)}
            ...
            <Link to="/profile" className='hover:text-black'>
             ...
             <button onClick={toggleCartDrawer} className='relative hover:text-black'>
                ...
                {cartItemsCount>0 &&(<span className='absolute -top-1  bg-rabbit-red text-white text-xs rounded-full px-2 py-0.5'>
                   {cartItemsCount}
                </span>)}
      ...
      transform transition-transform duration-300 z-50 ${navDrawerOpen ? "translate-x-0":"-translate-x-full"}`}>
        <div className="flex justify-end p-4">
          <button onClick={toggleNavDrawer}>
            <IoMdClose className='h-6 w-6 text-gary-600'/>
          </button>
        </div>
         <div className="p-4">
